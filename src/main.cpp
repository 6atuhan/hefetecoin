...
#if defined(NDEBUG)
# error "Hefetecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7b600c7bfabfcc12fbc09f925023ad3ef01c6d690a9b87be48bd9dd59b639167");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hefetecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hefetecoin Signed Message:\n";
...
{
    // Hefetecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Hefetecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Hefetecoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Hefetecoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hefetecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hefetecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe8d63f1d56bd4e78c34919a21b1bb1608d51f8fb7fb2677948e31d129df1c086");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "11.03.2022 - Hefetecoin'in ilk temelleri atıldı.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0410fe6d1fd58a94c62307effb0b458834729e44407872b8b472b13aaedbb6cac7e0fd0fdc9432d2c2a072f8628874267a174b00d1b71526ea0f16f2f938efc91f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1647010126;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086288652;

        if (fTestNet)
        {
            block.nTime    = 1647010073;
            block.nNonce   = 385816683;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x33ccbc6aa3b5026c9870f4a6dfafee109ec712ee20e3700df887139f8d13b54d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc8, 0xb9, 0xde }; // Hefetecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HefetecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HefetecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HefetecoinMiner(CWallet *pwallet)
{
    printf("HefetecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hefetecoin-miner");
    ...
    try { loop {
        ...
        printf("Running HefetecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HefetecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HefetecoinMiner, pwallet));
}
